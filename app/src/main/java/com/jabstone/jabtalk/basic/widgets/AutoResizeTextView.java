 ...
 * ellipsis.
 * 
 * 2011-10-29 changes by Alan Jay Weiner * change to fit both vertically and
 * horizontally * test mTextSize for 0 in resizeText() to fix exception in
 * Layout Editor
 * 
...
public class AutoResizeTextView extends TextView {
    ...
    protected void onLayout ( boolean changed, int left, int top, int right, int bottom ) {
        ...
        if(m_listener != null) {
        	m_listener.onAutoResizeLayoutChanged(right -left, bottom - top);
        }
    }
    ...
    public void resizeText ( int width, int height ) {
        ...
        int textWidth = getPreferredWidth ( targetTextSize );
        ...
        while ( ( ( textHeight > height ) || ( textWidth > width ) )
            ...
            textWidth = getPreferredWidth ( targetTextSize );
        }
    ...
    private int getPreferredWidth ( float fontSize ) {
        int wordSize = 0;
        TextPaint paint = new TextPaint ();
        Rect bounds = new Rect ();

        String longestWord = "";
        String words[] = getText ().toString ().split ( "\\s" );
